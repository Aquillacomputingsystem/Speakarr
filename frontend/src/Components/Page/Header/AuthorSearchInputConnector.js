...
function createMapDispatchToProps(dispatch, props) {
  return {
    onGoToAuthor(titleSlug) {
      dispatch(push(`${window.Speakarr.urlBase}/author/${titleSlug}`));
    },

    onGoToBook(titleSlug) {
      dispatch(push(`${window.Speakarr.urlBase}/book/${titleSlug}`));
    },

    onGoToAddNewAuthor(query) {
      dispatch(push(`${window.Speakarr.urlBase}/add/search?term=${encodeURIComponent(query)}`));
    }
  };
}
