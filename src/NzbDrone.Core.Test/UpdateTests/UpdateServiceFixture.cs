    ...
    public class UpdateServiceFixture : CoreTest<InstallUpdateService>
    {
        ...
        public void Setup()
        {
            if (OsInfo.IsLinux)
            {
                ...
                {
                    FileName = "Speakarr.develop.0.6.2.883.tar.gz",
                    Url = "https://github.com/speakarr/Speakarr/releases/download/v0.6.2.883/Speakarr.develop.0.6.2.883.linux.tar.gz",
            ...
            else
            {
                _updatePackage = new UpdatePackage
                {
                    FileName = "Speakarr.develop.0.6.2.883.zip",
                    Url = "https://github.com/speakarr/Speakarr/releases/download/v0.6.2.883/Speakarr.develop.0.6.2.883.windows.zip",
                    Version = new Version("0.6.2.883")
                };
            }

            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.TempFolder).Returns(TempFolder);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Speakarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\ProgramData\Speakarr".AsOsAgnostic);
            ...
            Mocker.GetMock<IRuntimeInfo>().Setup(c => c.ExecutingApplication).Returns(@"C:\Test\Speakarr.exe");
            ...
            Mocker.GetMock<IDiskProvider>()
                  .Setup(v => v.FileExists(It.Is<string>(s => s.EndsWith("Speakarr.Update".ProcessNameToExe()))))
                  .Returns(true);
        ...
        {
            const string scriptPath = "/tmp/speakarr/update.sh";
        ...
        public void Should_download_and_extract_to_temp_folder()
        {
            ...
            updateSubFolder.GetDirectories("Speakarr").Should().HaveCount(1);
        ...
        {
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.StartUpFolder).Returns(@"C:\Speakarr".AsOsAgnostic);
            Mocker.GetMock<IAppFolderInfo>().SetupGet(c => c.AppDataFolder).Returns(@"C:\Speakarr\AppData".AsOsAgnostic);
