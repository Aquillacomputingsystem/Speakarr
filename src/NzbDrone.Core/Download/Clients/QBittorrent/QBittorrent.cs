        ...
        private ValidationFailure TestConnection()
        {
            try
            {
                ...
                else if (Settings.MusicCategory.IsNullOrWhiteSpace())
                {
                    ...
                    {
                        IsWarning = true,
                        DetailedDescription = "Speakarr will not attempt to import completed downloads without a category."
                    };
                }
                ...
                if ((config.MaxRatioEnabled || config.MaxSeedingTimeEnabled) && (config.MaxRatioAction == QBittorrentMaxRatioAction.Remove || config.MaxRatioAction == QBittorrentMaxRatioAction.DeleteFiles))
                {
                    ...
                    {
                        DetailedDescription = "Speakarr will be unable to perform Completed Download Handling as configured. You can fix this in qBittorrent ('Tools -> Options...' in the menu) by changing 'Options -> BitTorrent -> Share Ratio Limiting' from 'Remove them' to 'Pause them'."
                    };
                }
            }
        ...
        private ValidationFailure TestCategory()
        {
            ...
            if (Settings.MusicCategory.IsNotNullOrWhiteSpace() && !labels.ContainsKey(Settings.MusicCategory))
            {
                ...
                if (!labels.ContainsKey(Settings.MusicCategory))
                {
                    ...
                    {
                        DetailedDescription = "Speakarr was unable to add the label to qBittorrent."
                    };
                }
            }
