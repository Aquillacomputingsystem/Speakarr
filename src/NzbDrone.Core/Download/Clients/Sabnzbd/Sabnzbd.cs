        ...
        private ValidationFailure TestConnectionAndVersion()
        {
            try
            {
                ...
                if (rawVersion.Equals("develop", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    {
                        IsWarning = true,
                        DetailedDescription = "Speakarr may not be able to support new features added to SABnzbd when running develop versions."
                    };
                }
        ...
        private ValidationFailure TestGlobalConfig()
        {
            ...
            if (config.Misc.pre_check && !HasVersion(1, 1))
            {
                return new NzbDroneValidationFailure("", "Disable 'Check before download' option in Sabnbzd")
                {
                    ...
                    DetailedDescription = "Using Check before download affects Speakarr ability to track new downloads. Also Sabnzbd recommends 'Abort jobs that cannot be completed' instead since it's more effective."
                };
            }
        ...
        private ValidationFailure TestCategory()
        {
            ...
            if (category != null)
            {
                if (category.Dir.EndsWith("*"))
                {
                    return new NzbDroneValidationFailure("MusicCategory", "Enable Job folders")
                    {
                        ...
                        DetailedDescription = "Speakarr prefers each download to have a separate folder. With * appended to the Folder/Path Sabnzbd will not create these job folders. Go to Sabnzbd to fix it."
                    };
                }
            }
            else
            ...
            if (config.Misc.enable_tv_sorting && ContainsCategory(config.Misc.tv_categories, Settings.MusicCategory))
            {
                return new NzbDroneValidationFailure("MusicCategory", "Disable TV Sorting")
                {
                    ...
                    DetailedDescription = "You must disable Sabnzbd TV Sorting for the category Speakarr uses to prevent import issues. Go to Sabnzbd to fix it."
                };
            }
            ...
            if (config.Misc.enable_movie_sorting && ContainsCategory(config.Misc.movie_categories, Settings.MusicCategory))
            {
                ...
                {
                    InfoLink = _proxy.GetBaseUrl(Settings, "config/sorting/"),
                    DetailedDescription = "You must disable Sabnzbd Movie Sorting for the category Speakarr uses to prevent import issues. Go to Sabnzbd to fix it."
                };
            }
            ...
            if (config.Misc.enable_date_sorting && ContainsCategory(config.Misc.date_categories, Settings.MusicCategory))
            {
                ...
                {
                    InfoLink = _proxy.GetBaseUrl(Settings, "config/sorting/"),
                    DetailedDescription = "You must disable Sabnzbd Date Sorting for the category Speakarr uses to prevent import issues. Go to Sabnzbd to fix it."
                };
            }
