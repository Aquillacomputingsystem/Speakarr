...
namespace NzbDrone.Update.UpdateEngine
{
    ...
    public class InstallUpdateService : IInstallUpdateService
    {
        ...
        public void Start(string installationFolder, int processId)
        {
            ...
            _logger.Info("Updating Speakarr from version {0} to version {1}", _detectExistingVersion.GetExistingVersion(installationFolder), BuildInfo.Version);

            Verify(installationFolder, processId);

            if (installationFolder.EndsWith(@"\bin\Speakarr") || installationFolder.EndsWith(@"/bin/Speakarr"))
            {
            ...
            _processProvider.FindProcessByName(ProcessProvider.SPEAKARR_CONSOLE_PROCESS_NAME);
            _processProvider.FindProcessByName(ProcessProvider.SPEAKARR_PROCESS_NAME);
            ...
            try
            {
                ...
                if (OsInfo.IsWindows)
                {
                    if (_processProvider.Exists(ProcessProvider.SPEAKARR_CONSOLE_PROCESS_NAME) || _processProvider.Exists(ProcessProvider.SPEAKARR_PROCESS_NAME))
                    {
                        _logger.Error("Speakarr was restarted prematurely by external process.");
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    {
                        _diskProvider.SetFilePermissions(Path.Combine(installationFolder, "Speakarr"), "755", null);
                    }
                }
                catch (Exception e)
                {
            ...
            finally
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < 10; i++)
                    {
                        ...
                        if (_processProvider.Exists(ProcessProvider.SPEAKARR_PROCESS_NAME))
                        {
                            _logger.Info("Speakarr was restarted by external process.");
                            break;
                        }
                    }
